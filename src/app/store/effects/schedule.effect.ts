...
export class ScheduleEffects {
  ...
  loadScheduleGroupUser$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ESchedule.LOAD_GROUP_USER),
      mergeMap((action: ScheduleGroupUser) =>
        this.scheduleService
          .loadScheduleGroupUser(
            action.payload.paginationGroupWeek,
            action.payload.paramsGroupWeek
          )
          .pipe(
            map(
              (res: Page<ListScheduleContent>) =>
                new ScheduleGroupUserSuccess({
                  paginationGroupWeek: {
                    page: res.page,
                    pageSize: res.pageSize,
                    total: res.total,
                  },
                  schedulesGroupWeek: this.convertToObj(res.data),
                  paramsGroupWeek: action.payload.paramsGroupWeek,
                })
            ),
            catchError((error) =>
              of(
                new ScheduleGroupUserFail({
                  message: MessageConstants.SCHEDULE_GROUP_USER_FAIL,
                })
              )
            )
          )
  ...
  convertToObj(data: ListScheduleContent[]): ListScheduleContent[] {
    data.forEach((e) => {
      e.detailUser = JSON.parse(String(e.detailUser));
      e.detailSchedule = JSON.parse(String(e.detailSchedule));
    });
    return data;
  }
}
